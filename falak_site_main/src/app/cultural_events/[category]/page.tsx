...
export function generateStaticParams() {
  return culturalCategories.map((c) => ({ category: c.slug }));
}

export default async function CulturalCategoryPage({
  params,
}: {
  params: Promise<{ category: string }>;
}) {
  const { category: categoryParam } = await params;
  const category = culturalCategories.find((c) => c.slug === categoryParam);
        ...
        {list.map((e) => (
          <div key={e.id} className="border rounded-lg p-4 space-y-1">
            ...
            <Link
              ...
              href={`/cultural_events/${category.slug}/${e.slug}`}
            >
              View
            </Link>
          </div>
