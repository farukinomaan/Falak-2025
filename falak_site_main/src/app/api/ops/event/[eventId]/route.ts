...
import { getRoleForEmail, opsEventRoster } from '@/lib/actions/adminAggregations';

export async function GET(_req: Request, context: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  const params = await context.params; // ensure awaited
  ...
  const res = await opsEventRoster(eventId);
