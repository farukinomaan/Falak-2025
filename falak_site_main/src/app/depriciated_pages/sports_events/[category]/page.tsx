...
export default async function SportsCategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const sports = events.filter((e) => (e.cluster_name || "").toLowerCase() === "sports");
  const list = sports.filter((e) => e.sub_cluster === category);
        ...
        {list.map((e) => (
          <div key={e.id} className="border rounded-lg p-4 space-y-1">
            ...
            <Link
              ...
              href={`/sports_events/${encodeURIComponent(category)}/${encodeURIComponent(e.id)}`}
            >
              View
            </Link>
          </div>
