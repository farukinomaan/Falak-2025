"use client";
// Generic code-based team registration UI (create / join via code) reused for MAHE users across all events.
// Replicates EsportsTeamRegistration but calls generalized codeTeams actions.
import React, { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { Copy } from "lucide-react";
import { toast } from "sonner";
import { createCodeTeam, joinCodeTeam, getCodeTeam } from "@/lib/actions/codeTeams";
...
export default function CodeTeamRegistration({ eventId, userId }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    startCreate(async () => {
      const res = await createCodeTeam({ eventId, captainId: userId, name: teamName.trim() });
  ...
  async function handleLookup(code: string) {
    ...
    const res = await getCodeTeam(trimmed);
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    startJoin(async () => {
      const res = await joinCodeTeam({ teamId: joinCode.trim(), userId });
      if (!res.ok) { toast.error(res.error); return; }
