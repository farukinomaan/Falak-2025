"use server";
// Generalized code-based team registration (create/join via shareable team ID) for ANY event.
// Mirrors esportsTeams.ts logic but without restricting sub_cluster = 'esports'.
// Capacity (max_team_size) is enforced the same way; uniqueness constraints rely on existing schema.
// NOTE: Esports single-event locking is still enforced at page level and NOT here.
import { getServiceClient } from "./supabaseClient";

export type ActionResult<T> = { ok: true; data: T } | { ok: false; error: string };

export async function createCodeTeam(params: { eventId: string; captainId: string; name: string }) {
...
export async function getCodeTeam(teamId: string) {
...
export async function joinCodeTeam(params: { teamId: string; userId: string }) {
  const { teamId, userId } = params;
  if (!teamId || !userId) return { ok: false as const, error: "Missing fields" };
  const supabase = getServiceClient();
  // Fetch team (and by extension event)
  const teamRes = await getCodeTeam(teamId);
  if (!teamRes.ok) return teamRes;
  const { eventId } = teamRes.data;
  // Capacity check identical to esports logic
  try {
  ...
  } catch { /* ignore capacity failures; allow join to proceed */ }
  // Check if user already captain this event
